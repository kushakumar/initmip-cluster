...
class FeatureSelector(pwidg.VBox):
    ...
    def __init__(self, dir_models: Path, **kwargs):
        ...
        # --- ACTIONS ---
        #Comment out disabling of 'Models' option for now
        ...
        def select_fields(btn):
            ...
            box_selectors.initialize_selection()
...
class FileSelectors(pwidg.HBox):
    ...
    def __init__(self, dir_models: Path, groups: dict = None, **kwargs):
        ...
        def initialize_selection():
            ...
            print(f"Initializing selection for experiment: {exp}")
            sel_fields.enable()
            filter = mfiles.filter_paths_terms(self._files_all, [exp], mfiles.FileParams.EXP)
            params = mfiles.union_netcdf_params(filter)
            print(f"Fields options: {sorted(params.fields)}")
        ...
        def select_exp(change):
            ...
            if exp:
                print(f"Experiment selected: {exp}")
                sel_fields.enable()
        ...
        def select_fields(change):
            ...
            if fields:
                ...
                print(f"Models options: {sorted(models)}")
        ...
        self.initialize_selection = initialize_selection
